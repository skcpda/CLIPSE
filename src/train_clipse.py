"""
Main training script for CLIP with SANW variants.
"""
import os
import math
import time
...
def build_model(model_id: str):
    ...
    if os.path.exists(model_id):
        ...
        model = CLIPModel.from_pretrained(model_id, use_safetensors=True, torch_dtype=torch.float32)
...
def train(cfg):
    ...
    # One-time diagnostics (opt-in)
    if cfg.get('diagnostics', False):
        try:
            n_train = sum(p.numel() for p in model.parameters() if p.requires_grad)
            n_total = sum(p.numel() for p in model.parameters())
            print(f"model in train: {model.training}")
            print(f"trainable params: {n_train}/{n_total}")
            for i, g in enumerate(optimizer.param_groups):
                sz = sum(p.numel() for p in g['params'])
                print(f"opt group {i}: lr={g.get('lr')}, params={sz}")
        except Exception:
            pass
    for epoch in range(1, cfg['epochs'] + 1):
        model.train()
        # Optionally freeze logit_scale for early epochs to prevent collapse
        freeze_epochs = int(cfg.get('freeze_logit_scale_epochs', 0) or 0)
        if epoch <= freeze_epochs:
            for p in [model.logit_scale]:
        ...
        for step, batch in enumerate(train_loader):
            ...
            # Gradient diagnostics (opt-in)
            if cfg.get('diagnostics', False) and (step + 1) % cfg.get('log_every', 20) == 0:
                try:
                    ...
                    print(f"Grad norms - Vision: {vision_grad_norm:.6f}, Text: {text_grad_norm:.6f}, LogitScale: {logit_scale_grad:.6f}")
                    ...
                    print(f"Has gradients - Vision: {vision_has_grad}, Text: {text_has_grad}")
                    
                except Exception as e:
                    print(f"Gradient diagnostic error: {e}")
            ...
            if (step + 1) % cfg.get('log_every', 20) == 0:
                ...
                # Feature similarity sanity (opt-in)
                if cfg.get('diagnostics', False):
                    try:
                        with torch.no_grad():
                            zi = F.normalize(model.get_image_features(pixel_values=batch["pixel_values"]), dim=-1)
                            zj = F.normalize(model.get_text_features(input_ids=batch["input_ids"], attention_mask=batch["attention_mask"]), dim=-1)
                            S = zi @ zj.t()
                            print(f"S mean/std: {S.mean().item():.4f}/{S.std().item():.4f}, diag mean: {S.diag().mean().item():.4f}")
                        # Gradient norms snapshot (after optimizer step previous grads cleared; compute on fresh backward next time)
                        # We approximate by checking param requires_grad flags
                        v_params = list(model.vision_model.parameters())
                        t_params = list(model.text_model.parameters())
                        v_req = sum(p.requires_grad for p in v_params)
                        t_req = sum(p.requires_grad for p in t_params)
                        print(f"vision params trainable: {v_req}/{len(v_params)}, text params trainable: {t_req}/{len(t_params)}")
                    except Exception:
                        pass
